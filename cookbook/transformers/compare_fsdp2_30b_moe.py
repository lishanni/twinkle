"""Qwen3-30B-A3B (MoE) deterministic comparison: native FSDP2 vs HyperParallel FSDP2.

Group 1 — Zero-error verification (blocking + deterministic):
    ASCEND_RT_VISIBLE_DEVICES=0,1,2,3 \\
    ASCEND_LAUNCH_BLOCKING=1 HCCL_DETERMINISTIC=true \\
    torchrun --nproc_per_node=4 cookbook/transformers/compare_fsdp2_30b_moe.py --backend native
    torchrun --nproc_per_node=4 cookbook/transformers/compare_fsdp2_30b_moe.py --backend hp

Group 2 — Performance comparison (HP with perf optimizations):
    ASCEND_RT_VISIBLE_DEVICES=0,1,2,3 \\
    torchrun --nproc_per_node=4 cookbook/transformers/compare_fsdp2_30b_moe.py --backend native
    torchrun --nproc_per_node=4 cookbook/transformers/compare_fsdp2_30b_moe.py --backend hp --perf
"""
import argparse
import os
...
def train():
    ...
    if args.backend == 'hp':
        ...
        if args.perf:
            hp_config.update({
                'comm_fusion': False,  # True OOMs on 30B-MoE (60/61 GB), even with seq_len=1024
    ...
    model.model._no_split_modules = {'Qwen3MoeDecoderLayer'}
    ...
    exp_label = f'GA={args.gradient_accumulation_steps}, lr={args.lr}, seq_len={args.max_length}'
    logger.info(f'Backend: {backend_label}{perf_label} (Qwen3-30B-A3B MoE, {exp_label})')
    ...
    log_memory('final')
    logger.info(f'{backend_label}{perf_label} training complete. ({exp_label})')
