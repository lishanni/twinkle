"""Profile native FSDP2 vs HyperParallel FSDP2 on Qwen3-30B-A3B (MoE) — single-step profiling at step 10.

Usage:
    torchrun --nproc_per_node=4 cookbook/transformers/profile_fsdp2_compare_30b_moe.py --backend native
    torchrun --nproc_per_node=4 cookbook/transformers/profile_fsdp2_compare_30b_moe.py --backend hp
"""
import argparse
import os
...
PROF_DIR = os.path.join(
    ...
    f'../../log/profiling/30b_moe_{"hp" if args.backend == "hp" else "native"}_fsdp2',
)
...
def train():
    ...
    logger.info(f'Backend: {backend_label} (Qwen3-30B-A3B MoE, profiling step {WARMUP_STEPS} only)')
