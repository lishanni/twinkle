"""Twinkle native FSDP2 baseline on NPU — same config as hyper_parallel_fsdp2_npu.py for comparison."""
import os
...
def log_memory(tag: str = '') -> None:
    ...
    if Torch.is_npu_available():
        ...
        logger.info(f'{prefix}GPU {local_rank} mem: alloc={allocated:.2f}GB, reserved={reserved:.2f}GB, peak={peak:.2f}GB')
...
def train():
    ...
    # Native FSDP2 (no hyper_parallel)
    ...
    logger.info(f'Backend: native FSDP2 (torch)')
    ...
    for step, batch in enumerate(dataloader):
        ...
        if step > 0 and step % 40 == 0:
            ...
            if best_loss > float(metrics['loss']):
                model.save(f'baseline-checkpoint-{step}')
                best_loss = float(metrics['loss'])
    model.save('baseline-last-checkpoint')
    log_memory('final')
