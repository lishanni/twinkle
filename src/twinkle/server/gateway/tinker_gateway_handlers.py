...
def _register_tinker_routes(app: FastAPI, self_fn) -> None:
    ...
    async def get_server_capabilities(request: Request, gw=Depends(self_fn)) -> types.GetServerCapabilitiesResponse:
        return types.GetServerCapabilitiesResponse(supported_models=gw.supported_models)
    ...
    async def create_session(request: Request, body: types.CreateSessionRequest,
                             gw=Depends(self_fn)) -> types.CreateSessionResponse:
        session_id = gw.state.create_session(body.model_dump())
        return types.CreateSessionResponse(session_id=session_id)

    @app.post('/session_heartbeat')
    async def session_heartbeat(
        request: Request, body: types.SessionHeartbeatRequest,
        gw=Depends(self_fn)) -> types.SessionHeartbeatResponse:  # noqa: E125
        alive = gw.state.touch_session(body.session_id)
        if not alive:
    ...
    async def create_sampling_session(
        request: Request, body: types.CreateSamplingSessionRequest,
        gw=Depends(self_fn)) -> types.CreateSamplingSessionResponse:  # noqa: E125
        sampling_session_id = gw.state.create_sampling_session(body.model_dump())
        return types.CreateSamplingSessionResponse(sampling_session_id=sampling_session_id)

    @app.post('/retrieve_future')
    async def retrieve_future(request: Request, body: types.FutureRetrieveRequest, gw=Depends(self_fn)) -> Any:
    ...
    async def publish_checkpoint(request: Request, run_id: str, checkpoint_id: str, gw=Depends(self_fn)) -> Response:
        token = get_token_from_request(request)
    ...
    async def save_weights_for_sampler(request: Request, body: types.SaveWeightsForSamplerRequest,
                                       gw=Depends(self_fn)) -> Any:
