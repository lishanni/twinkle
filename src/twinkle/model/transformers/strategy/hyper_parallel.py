...
class HyperParallelStrategy(AccelerateStrategy):
    ...
    def wrap_optimizer(self, optimizer):
        """Wrap optimizer.step with SkipDTensorDispatch for HP FSDP2."""
        _wrap_optimizer_step_with_skip_dtensor_dispatch(optimizer)
        return optimizer

    def log_device_memory(self, model, logger, tag: str = '') -> None:
        """Log device memory usage for current rank (NPU or GPU)."""
        ...
        if Torch.is_npu_available():
            ...
            logger.info(
                ...
                f'alloc={allocated:.2f}GB, resv={reserved:.2f}GB, peak={peak:.2f}GB'
            )
    ...
    def log_npu_memory(self, model, logger, tag: str = '') -> None:
        """Deprecated: Use log_device_memory instead."""
        self.log_device_memory(model, logger, tag)
