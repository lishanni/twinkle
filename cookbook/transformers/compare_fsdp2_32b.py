"""Qwen3-32B deterministic comparison: native FSDP2 vs HyperParallel FSDP2.

Group 1 — Zero-error verification (blocking + deterministic):
    ASCEND_RT_VISIBLE_DEVICES=0,1,2,3 \\
    ASCEND_LAUNCH_BLOCKING=1 HCCL_DETERMINISTIC=true \\
    torchrun --nproc_per_node=4 cookbook/transformers/compare_fsdp2_32b.py --backend native
    torchrun --nproc_per_node=4 cookbook/transformers/compare_fsdp2_32b.py --backend hp

Group 2 — Performance comparison (HP with perf optimizations):
    ASCEND_RT_VISIBLE_DEVICES=0,1,2,3 \\
    torchrun --nproc_per_node=4 cookbook/transformers/compare_fsdp2_32b.py --backend native
    torchrun --nproc_per_node=4 cookbook/transformers/compare_fsdp2_32b.py --backend hp --perf
"""
import argparse
import os
...
def train():
    ...
    if args.backend == 'hp':
        ...
        if args.perf:
            hp_config.update({
                'comm_fusion': False,  # True OOMs on 32B (59.7/61 GB), even with seq_len=1024
    ...
    logger.info(f'Backend: {backend_label}{perf_label} (Qwen3-32B, {exp_label})')
